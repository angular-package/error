...
export class ValidationError<Id extends string> extends CommonError<Id> {
   ...
   * @param template A template of error message with the replaceable `{problem}`, `{fix}` and optional `{id}`, `{min}`, `{max}` and tags.
   * By default, the value is picked from the static property `ValidationError.template`.
   ...
   * tags. By default, the value is picked from the static property `template`.
   * @angularpackage
   */
  constructor(
    problem: string,
